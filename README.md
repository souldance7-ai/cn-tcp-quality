...
- TCP 表逐项增加骨干路由型态；以 TCP traceroute 的 ASN 跳点判定电信 163／CN2 GT／CN2 GIA／CTGNet、联通 4837／9929／CUG 10099、移动 CMNET／CMI／CMIN2，证据不足时明确标为“未确定”
...
- 失败原因区分无 A／AAAA、连接超时、路径失效与服务拒绝；失败项目也会保留在终端表，完整诊断保存在 `endpoint-audit.csv`
...
- 直连测速先对传输端口做短预检；原公布端口失效时，同一已核对省份／运营商主机会继续验证 HTTP 80、HTTPS 443、HTTP 8080、HTTPS 8080，再兼容新版 `/download`、`/upload` 和传统 `random*.jpg`、`upload.php` 协议
...
测速脚本为十地区 30 个三网 IPv4 组合逐项寻找可用下载，并额外执行 1 组 IPv6 最近边缘测速。Zstatic 地址只承担 TCP SYN 品质探测，绝不被当作下载源；带宽只匹配对应地区和运营商的 Speedtest 候选。IPv6 全程强制解析原生 AAAA、绑定本机 IPv6 来源地址并使用 `curl -6`，不会用 IPv4 或 IPv4-mapped 地址替代。端点离线、没有对应地址族或下载低于有效门槛时仍会在终端显示失败状态，但不会写入成功结果 CSV；完整原因保存在 `endpoint-audit.csv`。仅下载成功、上传不可用时仍保留真实下载值并明确标注。
...
默认不执行吞吐测速。`--speed` 对十地区三网 IPv4 共 30 个组合逐项寻找可用端点，并另测 1 个自动路由的 IPv6 最近边缘端点；终端显示全部成功与失败项目，成功结果 CSV 只保留有真实下载数据的项目。脚本不设置 Mbps 上限，也不使用 Speedtest 省流模式；IPv4 标准模式每个直连方向最多 1GiB／8 秒、快速模式最多 256MiB／3 秒，IPv6 最近端点依服务规格单次最多下载 250 MB、上传 50 MB。端点拒绝或超时时会先结束该传输并尝试其他端口／协议，请确认 VPS 流量配额后使用。
