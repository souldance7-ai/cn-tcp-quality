...
- 北京／上海／广东 IPv4 优先使用真实 TOS 端点；安徽合肥三网优先直连已知运营商测速主机；其余组合同时从 Ookla 当前目录与每日更新的 Speedtest.cn 国内目录选择同省、同运营商候选
- 安徽按同省城市池回退：合肥电信 `17145`、合肥联通 `5724`、合肥移动 `26404／4377`，以及淮南移动 `34035`；先验证 Ookla `prod.hosts` 代理别名，再验证运营商原站与已知地址，不把“目录未收录”误判为“没有安徽测点”
...
测速脚本会为 30 个组合逐项发起测试，并绑定本机对应的 IPv4 或 IPv6 源地址。安徽合肥先验证内建的三网已知主机，再使用省会坐标和运营商关键词筛选 Ookla 目录，同时读取 Speedtest.cn 国内目录中明确标注的省份、运营商、下载及上传网址；IPv6 全程强制解析和连接 AAAA 地址，不依赖 Speedtest 核心是否支持 IPv6。若候选端点离线、没有对应地址族或只开放单向传输，会在该行明确显示原因，不用 IPv4 冒充 IPv6，也不生成假 Mbps。任一方向低于 0.1 Mbps 时不会误标为 `OK`。
...
默认不执行吞吐测速。`--speed` 会对五省三网 IPv4／IPv6 共 30 个组合逐项执行单线程下载与上传。标准模式每个直连方向最多 256MiB、快速模式每个方向最多 64MiB；端点拒绝或超时时会提前结束并尝试下一个候选。请确认 VPS 流量配额后使用。
